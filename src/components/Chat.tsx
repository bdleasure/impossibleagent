...
function ChatRoom({ roomId }: ChatProps) {
  ...
  const agent = useAgent({
    agent: "Chat", // Changed to match the agent name in server.ts
    name: `chat-${roomId}`,
  });

  const { messages, input, handleInputChange, handleSubmit: agentHandleSubmit, clearHistory } =
    useAgentChat({
      agent,
      maxSteps: 5,
    });

  // Custom handleSubmit to prevent default form behavior
